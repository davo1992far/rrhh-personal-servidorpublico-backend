...
{
    public class RabbitConsumerUnidadEjecutoraHostedService : AyniConsumerHostedService
    {
        public RabbitConsumerUnidadEjecutoraHostedService(IConfiguration configuration, IRabbitManagerConsumer rabbit) :
            base(configuration, rabbit, TipoConsumidor.ReplicaRegistro,
                Constante.RABBIT_EXCHANGE_NEGOCIO_COMUNES_MAESTROS_UNIDAD_EJECUTORA, Constante.RABBIT_QUEUE_NEGOCIO_COMUNES_MAESTROS_UNIDAD_EJECUTORA)
        {
        }
        ...
        protected override AcuseDeRecibo ConsumirMensaje(string contentJson)
        {
            ...
            try
            {
                ...
                var request = JsonSerializer.Deserialize<MensajeReplicaRegistro<UnidadEjecutoraReplica>>(contentJson);
                IUnidadEjecutoraService service = new UnidadEjecutoraService(config.GetConnectionString("DefaultConnection"));
