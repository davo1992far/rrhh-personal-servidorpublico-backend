...
{
    public class RabbitConsumerOtraInstanciaHostedService : AyniConsumerHostedService
    {
        public RabbitConsumerOtraInstanciaHostedService(IConfiguration configuration, IRabbitManagerConsumer rabbit) :
            base(configuration, rabbit, TipoConsumidor.ReplicaRegistro,
                Constante.RABBIT_EXCHANGE_NEGOCIO_COMUNES_MAESTROS_OTRA_INSTANCIA, Constante.RABBIT_QUEUE_NEGOCIO_COMUNES_MAESTROS_OTRA_INSTANCIA)
        {
        }
        ...
        protected override AcuseDeRecibo ConsumirMensaje(string contentJson)
        {
            ...
            try
            {
                ...
                var request = JsonSerializer.Deserialize<MensajeReplicaRegistro<OtraInstanciaReplica>>(contentJson);
                IOtraInstanciaService service = new OtraInstanciaService(config.GetConnectionString("DefaultConnection"));
