...
{
    public class RabbitConsumerNivelEducativoHostedService : AyniConsumerHostedService
    {

        public RabbitConsumerNivelEducativoHostedService(IConfiguration configuration, IRabbitManagerConsumer rabbit) :
            base(configuration, rabbit, TipoConsumidor.ReplicaRegistro,
                Constante.RABBIT_EXCHANGE_NEGOCIO_COMUNES_MAESTROS_NIVEL_EDUCATIVO, Constante.RABBIT_QUEUE_NEGOCIO_COMUNES_MAESTROS_NIVEL_EDUCATIVO)
        {
        }
        ...
        protected override AcuseDeRecibo ConsumirMensaje(string contentJson)
        {
            ...
            try
            {
                ...
                var request = JsonSerializer.Deserialize<MensajeReplicaRegistro<NivelEducativoReplica>>(contentJson);
                INivelEducativoService service = new NivelEducativoService(config.GetConnectionString("DefaultConnection"));
