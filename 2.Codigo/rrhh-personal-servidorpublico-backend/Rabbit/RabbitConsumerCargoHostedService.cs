...
{
    public class RabbitConsumerCargoHostedService : AyniConsumerHostedService
    {
        public RabbitConsumerCargoHostedService(IConfiguration configuration, IRabbitManagerConsumer rabbit) :
            base(configuration, rabbit, TipoConsumidor.ReplicaRegistro,
                Constante.RABBIT_EXCHANGE_NEGOCIO_COMUNES_MAESTROS_CARGO, Constante.RABBIT_QUEUE_NEGOCIO_COMUNES_MAESTROS_CARGO)
        {
        }
        ...
        protected override AcuseDeRecibo ConsumirMensaje(string contentJson)
        {
            ...
            try
            {
                ...
                var request = JsonSerializer.Deserialize<MensajeReplicaRegistro<CargoReplica>>(contentJson);
                ICargoService service = new CargoService(config.GetConnectionString("DefaultConnection"));
