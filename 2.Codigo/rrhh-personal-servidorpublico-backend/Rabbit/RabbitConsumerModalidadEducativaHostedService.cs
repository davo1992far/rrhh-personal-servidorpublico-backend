...
{
    public class RabbitConsumerModalidadEducativaHostedService : AyniConsumerHostedService
    {
        public RabbitConsumerModalidadEducativaHostedService(IConfiguration configuration, IRabbitManagerConsumer rabbit) :
            base(configuration, rabbit, TipoConsumidor.ReplicaRegistro,
                Constante.RABBIT_EXCHANGE_NEGOCIO_COMUNES_MAESTROS_MODALIDAD_EDUCATIVA, Constante.RABBIT_QUEUE_NEGOCIO_COMUNES_MAESTROS_MODALIDAD_EDUCATIVA)
        {
        }
        ...
        protected override AcuseDeRecibo ConsumirMensaje(string contentJson)
        {
            ...
            try
            {
                ...
                var request = JsonSerializer.Deserialize<MensajeReplicaRegistro<ModalidadEducativaReplica>>(contentJson);
                IModalidadEducativaService service = new ModalidadEducativaService(config.GetConnectionString("DefaultConnection"));
