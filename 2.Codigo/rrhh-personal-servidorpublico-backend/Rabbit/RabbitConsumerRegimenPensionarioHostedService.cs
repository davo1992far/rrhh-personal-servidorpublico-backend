...
{
    public class RabbitConsumerRegimenPensionarioHostedService : AyniConsumerHostedService
    {
        public RabbitConsumerRegimenPensionarioHostedService(IConfiguration configuration, IRabbitManagerConsumer rabbit) :
            base(configuration, rabbit, TipoConsumidor.ReplicaRegistro,
                Constante.RABBIT_EXCHANGE_NEGOCIO_COMUNES_MAESTROS_REGIMEN_PENSIONARIO, Constante.RABBIT_QUEUE_NEGOCIO_COMUNES_MAESTROS_REGIMEN_PENSIONARIO)
        {
        }
        ...
        protected override AcuseDeRecibo ConsumirMensaje(string contentJson)
        {
            ...
            try
            {
                ...
                var request = JsonSerializer.Deserialize<MensajeReplicaRegistro<RegimenPensionarioReplica>>(contentJson);
                IRegimenPensionarioService service = new RegimenPensionarioService(config.GetConnectionString("DefaultConnection"));
