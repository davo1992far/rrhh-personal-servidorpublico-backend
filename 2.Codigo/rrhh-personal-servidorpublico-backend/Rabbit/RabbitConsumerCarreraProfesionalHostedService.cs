...
{
    public class RabbitConsumerCarreraProfesionalHostedService : AyniConsumerHostedService
    {
        public RabbitConsumerCarreraProfesionalHostedService(IConfiguration configuration, IRabbitManagerConsumer rabbit) :
            base(configuration, rabbit, TipoConsumidor.ReplicaRegistro,
                Constante.RABBIT_EXCHANGE_NEGOCIO_COMUNES_MAESTROS_CARRERA_PROFESIONAL, Constante.RABBIT_QUEUE_NEGOCIO_COMUNES_MAESTROS_CARRERA_PROFESIONAL)
        {
        }
        ...
        protected override AcuseDeRecibo ConsumirMensaje(string contentJson)
        {
            ...
            try
            {
                ...
                var request = JsonSerializer.Deserialize<MensajeReplicaRegistro<CarreraProfesionalReplica>>(contentJson);
                ICarreraProfesionalService service = new CarreraProfesionalService(config.GetConnectionString("DefaultConnection"));
