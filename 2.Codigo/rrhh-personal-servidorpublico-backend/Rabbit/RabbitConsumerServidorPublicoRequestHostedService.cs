...
{
    public class RabbitConsumerServidorPublicoRequestHostedService: AyniConsumerHostedService
    {
        public RabbitConsumerServidorPublicoRequestHostedService(IConfiguration configuration, IRabbitManagerConsumer rabbit) :
            base(configuration, rabbit, TipoConsumidor.ReplicaRegistro,
                Constante.RABBIT_EXCHANGE_SERVIDORPUBLICO, Constante.RABBIT_QUEUE_SERVIDORPUBLICO)
        {
        }
        ...
        protected override AcuseDeRecibo ConsumirMensaje(string contentJson)
        {
            ...
            try
            {
                ...
                var request = JsonSerializer.Deserialize<MensajeReplicaRegistro<ServidorPublicoReplica>>(contentJson);
