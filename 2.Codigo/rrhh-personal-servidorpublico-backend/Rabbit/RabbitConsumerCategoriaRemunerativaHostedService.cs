...
{
    public class RabbitConsumerCategoriaRemunerativaHostedService: AyniConsumerHostedService
    {
        public RabbitConsumerCategoriaRemunerativaHostedService(IConfiguration configuration, IRabbitManagerConsumer rabbit) :
            base(configuration, rabbit, TipoConsumidor.ReplicaRegistro,
                Constante.RABBIT_EXCHANGE_NEGOCIO_COMUNES_MAESTROS_CATEGORIA_REMUNERATIVA, Constante.RABBIT_QUEUE_NEGOCIO_COMUNES_MAESTROS_CATEGORIA_REMUNERATIVA)
        {
        }
        ...
        protected override AcuseDeRecibo ConsumirMensaje(string contentJson)
        {
            ...
            try
            {
                ...
                var request = JsonSerializer.Deserialize<MensajeReplicaRegistro<CategoriaRemunerativaReplica>>(contentJson);
                ICategoriaRemunerativaService service = new CategoriaRemunerativaService(config.GetConnectionString("DefaultConnection"));
