...
{
    public class RabbitConsumerAfpHostedService: AyniConsumerHostedService
    {

        public RabbitConsumerAfpHostedService(IConfiguration configuration, IRabbitManagerConsumer rabbit) :
            base(configuration, rabbit, TipoConsumidor.ReplicaRegistro,
                Constante.RABBIT_EXCHANGE_NEGOCIO_COMUNES_MAESTROS_AFP, Constante.RABBIT_QUEUE_NEGOCIO_COMUNES_MAESTROS_AFP)
        {
        }
        ...
        protected override AcuseDeRecibo ConsumirMensaje(string contentJson)
        {
            ...
            try
            {
                ...
                var request = JsonSerializer.Deserialize<MensajeReplicaRegistro<AfpReplica>>(contentJson);
                IAfpService service = new AfpService(config.GetConnectionString("DefaultConnection"));
