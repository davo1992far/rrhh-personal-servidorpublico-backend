...
{
    public class RabbitConsumerJornadaLaboralHostedService: AyniConsumerHostedService
    {

        public RabbitConsumerJornadaLaboralHostedService(IConfiguration configuration, IRabbitManagerConsumer rabbit) :
            base(configuration, rabbit, TipoConsumidor.ReplicaRegistro,
                Constante.RABBIT_EXCHANGE_NEGOCIO_COMUNES_MAESTROS_JORNADA_LABORAL, Constante.RABBIT_QUEUE_NEGOCIO_COMUNES_MAESTROS_JORNADA_LABORAL)
        {
        }
        ...
        protected override AcuseDeRecibo ConsumirMensaje(string contentJson)
        {
            ...
            try
            {
                ...
                var request = JsonSerializer.Deserialize<MensajeReplicaRegistro<JornadaLaboralReplica>>(contentJson);
                IJornadaLaboralService service = new JornadaLaboralService(config.GetConnectionString("DefaultConnection"));
