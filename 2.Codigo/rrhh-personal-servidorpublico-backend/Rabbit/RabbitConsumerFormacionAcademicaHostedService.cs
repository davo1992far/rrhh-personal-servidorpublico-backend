...
{
    public class RabbitConsumerFormacionAcademicaHostedService: AyniConsumerHostedService
    {
        public RabbitConsumerFormacionAcademicaHostedService(IConfiguration configuration, IRabbitManagerConsumer rabbit) :
            base(configuration, rabbit, TipoConsumidor.ReplicaRegistro,
                Constante.RABBIT_EXCHANGE_NEGOCIO_COMUNES_MAESTROS_FORMACION_ACADEMICA, Constante.RABBIT_QUEUE_NEGOCIO_COMUNES_MAESTROS_FORMACION_ACADEMICA)
        {
        }
        ...
        protected override AcuseDeRecibo ConsumirMensaje(string contentJson)
        {
            ...
            try
            {
                ...
                var request = JsonSerializer.Deserialize<MensajeReplicaRegistro<FormacionAcademicaReplica>>(contentJson);
                IFormacionAcademicaService service = new FormacionAcademicaService(config.GetConnectionString("DefaultConnection"));
