...
{
    public class RabbitConsumerGradoInstruccionHostedService : AyniConsumerHostedService
    {
        public RabbitConsumerGradoInstruccionHostedService(IConfiguration configuration, IRabbitManagerConsumer rabbit) :
            base(configuration, rabbit, TipoConsumidor.ReplicaRegistro,
                Constante.RABBIT_EXCHANGE_NEGOCIO_COMUNES_MAESTROS_GRADO_INSTRUCCION, Constante.RABBIT_QUEUE_NEGOCIO_COMUNES_MAESTROS_GRADO_INSTRUCCION)
        {
        }
        ...
        protected override AcuseDeRecibo ConsumirMensaje(string contentJson)
        {
            ...
            try
            {
                ...
                var request = JsonSerializer.Deserialize<MensajeReplicaRegistro<GradoInstruccionReplica>>(contentJson);
                IGradoInstruccionService service = new GradoInstruccionService(config.GetConnectionString("DefaultConnection"));
