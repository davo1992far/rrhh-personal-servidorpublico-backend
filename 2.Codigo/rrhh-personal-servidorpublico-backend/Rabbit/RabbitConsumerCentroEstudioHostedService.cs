...
{
    public class RabbitConsumerCentroEstudioHostedService : AyniConsumerHostedService
    {
        public RabbitConsumerCentroEstudioHostedService(IConfiguration configuration, IRabbitManagerConsumer rabbit) :
            base(configuration, rabbit, TipoConsumidor.ReplicaRegistro,
                Constante.RABBIT_EXCHANGE_NEGOCIO_COMUNES_MAESTROS_CENTRO_ESTUDIO, Constante.RABBIT_QUEUE_NEGOCIO_COMUNES_MAESTROS_CENTRO_ESTUDIO)
        {
        }
        ...
        protected override AcuseDeRecibo ConsumirMensaje(string contentJson)
        {
            ...
            try
            {
                ...
                var request = JsonSerializer.Deserialize<MensajeReplicaRegistro<CentroEstudioReplica>>(contentJson);
                ICentroEstudioService service = new CentroEstudioService(config.GetConnectionString("DefaultConnection"));
